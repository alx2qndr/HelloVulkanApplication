...
private:
    ...
    void createWindow()
    {
        if (!SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS)) {
            throw std::runtime_error("Failed to initialize! SDL error: %s\n" + std::string(SDL_GetError()));
        }

        if (!SDL_Vulkan_LoadLibrary(nullptr)) {
            throw std::runtime_error("Failed to load Vulkan library! SDL error: %s\n" + std::string(SDL_GetError()));
        }

        window = SDL_CreateWindow("Hello, Vulkan Application!", WINDOW_WIDTH, WINDOW_HEIGHT,
            SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE);
        if (window == nullptr) {
            throw std::runtime_error("Failed to create window! SDL error: %s\n" + std::string(SDL_GetError()));
        }
    ...
    void createSurface()
    {
        if (!SDL_Vulkan_CreateSurface(window, instance, nullptr, &surface)) {
            throw std::runtime_error("Failed to create window surface! SDL error: %s\n" + std::string(SDL_GetError()));
        }
    }
    ...
    std::vector<const char*> getRequiredExtensions()
    {
        ...
        const char* const* extensionsNames = SDL_Vulkan_GetInstanceExtensions(&extensionCount);
